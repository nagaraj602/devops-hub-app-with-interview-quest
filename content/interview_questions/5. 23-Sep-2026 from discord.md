...
**Answer:**
Performing a zero-downtime Kubernetes cluster upgrade—specifically on an **AWS EKS** cluster managed via **Terraform**—requires a structured, phased approach. Because Kubernetes follows a strict version skew policy (components can differ by at most one minor version), upgrades must be conducted sequentially (e.g., from `1.29` to `1.30`, never skipping minor versions).

Zero downtime is achieved by ensuring high availability at the application layer, upgrading the managed control plane first, updating cluster add-ons, and utilizing a **Blue/Green Node Group upgrade** (or rolling cordon-and-drain update) so application pods migrate seamlessly to new worker nodes with zero dropped user requests.

---

### 1. Phase 1: Pre-Upgrade Planning & Backup

Before touching production infrastructure, execute readiness verifications:
1. **API Deprecations Audit:** Run tools like `kube-no-trouble` (`kubent`) or `pluto` to identify any manifests or Helm releases utilizing deprecated or removed APIs in the target Kubernetes version:
   ```bash
   kubent -t 1.30.0
   ```
2. **Verify Pod Disruption Budgets (PDB):** Ensure all production workloads have a configured `PodDisruptionBudget` (`minAvailable` or `maxUnavailable`) and run with at least 2 replicas distributed across multiple Availability Zones with `podAntiAffinity`:
   ```bash
   kubectl get pdb -A
   ```
3. **Cluster State & Database Backup:**
   - Execute a complete cluster snapshot using **Velero** (captures all Kubernetes resources, CRDs, secrets, and PersistentVolume snapshots):
     ```bash
     velero backup create pre-upgrade-v129-backup --include-namespaces='*'
     ```
   - Take snapshot backups of all stateful backend databases (AWS RDS, Aurora) and commit/tag the existing Terraform configuration repository.

---

### 2. Phase 2: Upgrading the AWS EKS Control Plane (via Terraform)

AWS manages the high availability of the EKS control plane (API servers and etcd) across multiple availability zones. Upgrading the control plane does not interrupt running worker nodes or application pods:

1. Update the `cluster_version` attribute in your Terraform code:
   ```hcl
   module "eks" {
     source          = "terraform-aws-modules/eks/aws"
     version         = "~> 20.0"
     cluster_name    = "prod-eks-cluster"
     cluster_version = "1.30"   # Upgraded from 1.29
     # ...
   }
   ```
2. Execute `terraform plan` and `terraform apply`.
3. EKS performs a rolling update of the API server instances behind AWS load balancers. Verify health:
   ```bash
...
### 3. Phase 3: Upgrading Core Cluster Add-ons

Upgrade the foundational networking and storage add-ons to versions compatible with Kubernetes 1.30:
...
cluster_addons = {
  ...
  kube-proxy = { addon_version = "v1.30.0-eksbuild.1" }
...
### 4. Phase 4: Zero-Downtime Worker Node Upgrade (Blue/Green Strategy)

To achieve zero downtime with zero risk, use the **Blue/Green Node Group** pattern in Terraform:

```
Step 1: Existing (Blue)                 Step 2: Deploy New (Green)              Step 3: Cordon & Drain Blue
+-----------------------------+         +-----------------------------+         +-----------------------------+
| Node Group Blue (v1.29)     |         | Node Group Blue (v1.29)     |         | Node Group Blue (v1.29)     |
| [ Pod A ] [ Pod B ] [ Pod C]|         | [ Pod A ] [ Pod B ] [ Pod C]|         | (Cordoned / Draining...)    |
+-----------------------------+         +-----------------------------+         +-----------------------------+
                                                       |                                       |
                                                       v                                       v (Pods migrate)
                                        +-----------------------------+         +-----------------------------+
                                        | Node Group Green (v1.30)    |         | Node Group Green (v1.30)    |
                                        | (Empty / Ready for pods)    |         | [ Pod A ] [ Pod B ] [ Pod C]|
                                        +-----------------------------+         +-----------------------------+
```

1. **Deploy Green Node Group:** Add a new node group in Terraform matching version `1.30` while leaving the existing `1.29` node group running:
   ```hcl
   ...
   ```
2. Apply Terraform to launch the new nodes. Wait until all Green nodes show `Ready`:
   ```bash
   kubectl get nodes -o wide
   ```
3. **Cordon the Old (Blue) Nodes:** Prevents new pods from scheduling on the old nodes:
   ```bash
   kubectl cordon -l eks.amazonaws.com/nodegroup=nodegroup-v129
   ```
4. **Drain the Old (Blue) Nodes Gracefully:**
   ```bash
   kubectl drain -l eks.amazonaws.com/nodegroup=nodegroup-v129 \
     --ignore-daemonsets \
     --delete-emptydir-data \
     --grace-period=60
   ```
   *Because Pod Disruption Budgets (PDB) are enforced, `kubectl drain` evicts pods sequentially. New pods automatically spin up on the Green 1.30 nodes without service disruption.*
5. **Decommission Blue Node Group:** Remove `blue_v129` from Terraform and run `terraform apply` to terminate old EC2 instances cleanly.

---

### 5. Rollback & Disaster Recovery Procedures

| Failure Scenario | Rollback & Recovery Action |
| :--- | :--- |
| **Control Plane Upgrade Failure** | AWS EKS manages control plane rollouts transactionally. However, AWS does **not** support downgrading an EKS control plane version. If unexpected issues arise, keep the control plane at target version and address worker/add-on issues. |
| **Worker Node Group Migration Failure** | Cancel node draining immediately (`kubectl uncordon`). The Blue node group is still active and serving traffic. Terminate Green nodes in Terraform, debug launch templates, and retry. |
| **Application Crash on New Version** | Revert application image or deployment manifests using Helm / Argo CD. If cluster state is corrupted, restore manifests and persistent volumes from the **Velero backup**. |
| **Catastrophic Cluster Failure** | Deploy a parallel fresh EKS cluster on the previous stable version using Terraform modules and restore workloads via Velero, redirecting traffic via Route 53 DNS weighted routing. |

---

### Operational Tips & Troubleshooting
1. **Ingress Controller Resilience:** Ensure ingress controllers (AWS Load Balancer Controller or Nginx Ingress) run with at least 2 replicas with `podAntiAffinity` across availability zones so external traffic routing remains uninterrupted during node drains.
2. **Drain Timeouts:** If `kubectl drain` hangs on a pod, inspect if the pod lacks a PDB allowing eviction, has local storage locks, or is an unmanaged bare pod created without a Deployment/ReplicaSet controller.
</details>
